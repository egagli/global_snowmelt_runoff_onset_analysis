...
| `case_studies/sierra_nevada/` | `sierra_nevada` | the dataset store (4x coarsened), the ERA5-Land stack, the hillshade, forest cover (FCF ≤ 50 % masked like the pipeline) | `sierra_nevada_*.png` (median, MAD, anomalies, spring temperature, scatter, legend) and the assembled `sierra_nevada_worked_example.png` |
| `climate/` | `pixelwise_climate_correlations` | ERA5-Land stack and the public pyramid (Equal Earth derived on the fly) | a correlations zarr under `scratch/` (local, exploratory) |
